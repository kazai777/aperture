...
include "./cipher_constants.circom";
...
template CPoseidonHashUnused(nInputs) {
    signal input inputs[nInputs];
    signal output out;

    component strategy = CPoseidonPerm(nInputs + 1);
    strategy.inputs[0] <== 0;
...
template CPoseidonPerm(t) {
    ...
    var C[t*(nRoundsF + nRoundsP)] = CIPHER_POSEIDON_C(t);
    var M[t][t] = CIPHER_POSEIDON_M(t);
...
template CPoseidonDecrypt(l) {
    ...
    component iterations = CPoseidonDecryptIterations(l);
...
template CPoseidonDecryptWithoutCheck(l) {
    var decryptedLength = l;
    while (decryptedLength % 3 != 0) {
        decryptedLength += 1;
    }
...
template CPoseidonDecryptIterations(l) {
    var decryptedLength = l;
    while (decryptedLength % 3 != 0) {
        decryptedLength += 1;
    }
    ...
    // Iterate CPoseidonHashUnused on the initial state
    strategies[0] = CPoseidonPerm(4);
    ...
    for (var i = 0; i < n; i ++) {
        ...
        // Iterate CPoseidonHashUnused on the state
        strategies[i + 1] = CPoseidonPerm(4);
